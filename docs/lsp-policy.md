...
- `textDocument/documentSymbol` (file outline / Go to Symbol)
...
3. A server may start only when the user explicitly requests definition, references, or document symbols for a supported language.
...
- Stop idle language servers after 5 minutes without definition, references, or document-symbol requests.
...
- `definition`, `references`, and `documentSymbol` are the only IPC navigation methods.
...
- The LSP client uses minimal JSON-RPC stdio framing for `initialize`, `initialized`, `textDocument/didOpen`, `textDocument/definition`, `textDocument/references`, and `textDocument/documentSymbol`.
- Definition and references responses are converted from LSP file URI locations into ArchSight root/path/range locations; documentSymbol responses (hierarchical `DocumentSymbol[]` or flat `SymbolInformation[]`) are flattened into name/kind/line/column/depth entries.
